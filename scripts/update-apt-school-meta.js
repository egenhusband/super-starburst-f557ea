...
async function main() {
  ...
  for (const target of scopedTargets) {
    ...
    try {
      ...
      let place = pickBestPlace(target, Array.isArray(placeResponse?.documents) ? placeResponse.documents : []);
      if (!place && target.doroJuso) {
        await sleep(REQUEST_DELAY_MS);
        const addressResponse = await fetchKakaoJson(buildAddressSearchUrl(target.doroJuso));
        place = normalizeAddressResult(Array.isArray(addressResponse?.documents) ? addressResponse.documents[0] : null);
      }
      if (!place) {
        skipped += 1;
        continue;
